...
int main() { 
    ...
    Size img_size; int cnt = 0; 
    int k = 0, n = 0; 
    std::string path0 = "/home/nvidia/gitee/rb_vm_ubuntu/hw2/calib2/";
    for (int i = 0; i < 20; i++){ 
        cv::Mat src0 = cv::imread(path0 + std::__cxx11::to_string(i).append("_orig.jpg")); 
        if ( !cnt ) { img_size.width = src0.cols; img_size.height = src0.rows; }
        found = findChessboardCorners( src0, board_size, point_pix_pos_buf ); 
        if ( found && point_pix_pos_buf.size() == board_n ) { 
            successes++; cvtColor( src0, gray_img, COLOR_BGR2GRAY ); 
            ...
            imshow( "corners", drawn_img ); waitKey( 50 ); 
            } else
            std::cout << "\tbut failed to found all chess board corners i n this image" << std::endl; 
...
for (int i = 0; i < successes; i++ ) { 
    for (int j = 0; j < board_h; j++ ) { 
        for (int k = 0; k < board_w; k++ ){ 
            ...
            pt.z = 0; point_grid_pos_buf.push_back( pt ); 
        } 
    }
    ...
    std::cout << camera_matrix << std::endl << dist_coeffs << std::endl; return 0; }